...
namespace BattleSnake.InternalModel {
    public interface IFastModelController {

        string Name { get; }

        void Start(FastWorld w, int ownIndex);

        Direction Move(FastWorld w, int ownIndex, DeepeningSearch.SearchLimit limit);

        void End(FastWorld w, int ownIndex);
    }
}